...
#        model = TCN(input_size=1, output_size=33, num_channels=[32] *12, kernel_size=2)
    ...
    train_labeled_dataset, train_unlabeled_dataset, val_dataset,transductive_dataset = get_tcga('./data', args.n_labeled,  transform_train=transform_train, transform_val=transform_val)
    ...
    transductive_loader = data.DataLoader(transductive_dataset, batch_size=args.unlabeled_batch_size, shuffle=False,
    ...
    if args.resume:
        ...
        logger.set_names(['epoch', 'Train_class_loss',  'Train_consistency_loss', 'Val_Loss', 'Val_Acc.', 'T_Loss', 'T_Acc.'])
    ...
    if args.evaluate:
        ...
        return
    all_labels =  np.zeros([len(train_unlabeled_dataset),33])
    for epoch in range(args.start_epoch, args.epochs):
        ...
        if args.evaluation_epochs and (epoch + 1) % args.evaluation_epochs == 0:
            ...
            t_loss, t_acc = validate(transductive_loader, model, criterion, epoch)
            print("--- validation in %s seconds ---" % (time.time() - start_time))
            logger.append([epoch, class_loss, cons_loss, val_loss, val_acc,t_loss,t_acc])

            print("Evaluating the EMA model:")
            val_loss, val_acc = validate(eval_loader, ema_model, criterion, epoch)
            t_loss, t_acc = validate(transductive_loader, ema_model, criterion, epoch)
            print("--- validation in %s seconds ---" % (time.time() - start_time))
            logger.append([epoch, class_loss, cons_loss, val_loss, val_acc, t_loss, t_acc])
