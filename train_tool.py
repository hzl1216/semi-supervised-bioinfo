...
from util.net import WideResNet,TCN, ResNet50,Vae
...
def train_semi(train_labeled_loader, train_unlabeled_loader, model, ema_model, vae,optimizer,ema_optimizer, vae_optimizer,epoch,scheduler=None):
    ...
    for i in range(args.epoch_iteration):
        ...
        encode_u ,outputs_u = model(inputs_u)
        encode_u  = Variable(encode_u.detach().data, requires_grad=False)
        decode_u ,mu, logvar = vae(encode_u)

        with torch.no_grad():
            ema_inputs_u = inputs_u.cuda()
            _, targets_u = model(ema_inputs_u)
            targets_u = torch.softmax(targets_u, dim=1)
            if epoch>args.vae_step:
                vae_samples = Variable(decode_u.detach().data, requires_grad=False)
                _,outputs_vae = model(vae_samples)
                targets_u = (torch.softmax(outputs_vae, dim=1) + targets_u)/2
                if args.mixup:
                    targets_u = sharpen(targets_u)
                vae_samples = vae_samples.detach()
            else:
                if args.mixup:
                    targets_u = sharpen(targets_u)
        ...
        if args.mixup:
            ...
            if epoch<=args.vae_step:
                ...
                all_inputs = torch.cat([inputs_x, inputs_u,vae_samples], dim=0)
            ...
            loss,class_loss ,consistency_loss = semiLoss(outputs_x, targets_x, outputs_u, targets_u, class_criterion, consistency_criterion,epoch)
        v_loss = vae_loss(decode_u,inputs_u,mu,logvar)
        meters.update('loss', loss.item())
        meters.update('class_loss', class_loss.item())
        meters.update('cons_loss', consistency_loss.item())
        meters.update('v_loss', v_loss.item()/batch_size)
