...
def train_semi(train_labeled_loader, train_unlabeled_loader, model, ema_model, optimizer,ema_optimizer,all_labels, epoch, scheduler=None):
    ...
    for i in range(args.epoch_iteration):
        ...
        if args.mixup:
            ...
            loss,class_loss ,consistency_loss = semiloss_mixup(outputs_x, targets_x, outputs_u1, outputs_u2,epoch+i/args.epoch_iteration)
        else:
            outputs_x = model(inputs_x)
            outputs_u = model(inputs_u1)
            loss,class_loss ,consistency_loss = semiLoss(outputs_x, targets_x, outputs_u, outputs_u1, class_criterion, consistency_criterion,epoch+i/args.epoch_iteration)
